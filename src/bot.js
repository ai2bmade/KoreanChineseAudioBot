...
function bottomRankMessage(direction, fromBottom, totalPlayers) {
  ...
  return `今天这个练习组共有 ${totalPlayers} 名学习者，你的成绩是倒数第 ${fromBottom} 名。`;
}

function challengeResultMessage(score, total, direction) {
  if (score === total) {
    return direction === "ru_ko" ? ["완벽합니다! 모두 맞혔습니다."] : ["完美！全部答对了。"];
  }

  if (score === 0) {
    return direction === "ru_ko" ? ["아이고... 0개 맞혔습니다."] : ["天哪……你一个都没答对。"];
  }
...
function challengeKeyboard() {
  return Markup.inlineKeyboard([
    [Markup.button.callback("Korean -> Chinese", "challenge_ko_ru")],
    [Markup.button.callback("Chinese -> Korean", "challenge_ru_ko")],
    [Markup.button.callback("Next Practice", "practice")]
  ]);
}
...
async function sendChallengeQuestion(ctx, chatId) {
  ...
  const prompt =
    session.direction === "ko_ru"
      ? ["韩语 -> 中文", `Задание: ${expression.ko}`, "下面两个选项中，哪一个意思相同？"]
      : ["중국어 -> 한국어", `문제: ${expression.foreign}`, "다음 둘 중 문제와 같은 뜻은?"];
...
bot.start(async (ctx) => {
  await ctx.reply(
    [
      "你好！",
      "",
      "这个机器人是为想学习韩语的中文使用者制作的。",
      "同时，它也适合正在学习中文的韩国人。",
      "",
      "每个音频文件都包含韩语表达和中文表达，同一个表达会重复20次以上。",
      "想再听一遍时，只要再次点击音频文件即可。",
      "",
      "免费用户每天可以听3个表达。",
      "如果你通过 Buy Me a Coffee 支持 $5，就可以获得3个月的访问权限，并且不限次数练习各种表达。",
      "",
      "如果你想练习韩语听写，请到这里：",
      "https://t.me/KoreanListeningBot",
      "",
      ...
      "안녕하세요!",
      "",
      "이 봇은 한국어를 배우고 싶은 중국어권 학습자를 위한 듣기 연습 봇입니다. 동시에 중국어를 배우는 한국인에게도 도움이 되도록 만들었습니다.",
      "",
